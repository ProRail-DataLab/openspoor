...
class SpoortakModelsData(Singleton):
    ...
    def _replace_km_columns(df: pd.DataFrame) -> None:
        ...
        df_changed_geo = df_changed[
            df_changed["GEOCODE_BEGIN"] != df_changed["GEOCODE_EIND"]
        ]
    ...
    def __init__(self, data_path: str):
        ...
        if not self.models:
            ...
            self.model_version_numbers = [
                version
                for version in self.model_version_numbers
                if version < unsupported_version_start
            ]

            try:
                for model_version in self.model_version_numbers:
                    self.models[model_version] = pd.read_csv(
                        os.path.join(
                            data_path,
            ...
            self.model_changes = {
                model_version: pd.read_csv(
                    os.path.join(
                        data_path,
                        f"Versie_{model_version:02d}",
                        f"BBMS_BERICHT_{model_version}.csv",
                    ),
                    delimiter=";",
                    header=0,
                    encoding="latin1",
