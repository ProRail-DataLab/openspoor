...
class SpoortakModelInspector:
    ...
    def _set_pd_options(self):
        ...
        self.old_pd_option_values["display.max_columns"] = pd.get_option(
            "display.max_columns"
        )
        self.old_pd_option_values["display.max_rows"] = pd.get_option(
            "display.max_rows"
        )
        self.old_pd_option_values["display.expand_frame_repr"] = pd.get_option(
            "display.expand_frame_repr"
        )
        self.old_pd_option_values["mode.chained_assignment"] = pd.get_option(
    ...
    def inspect(self, spoortak_identifier: str):
        ...
        model_data = pd.DataFrame(
            columns=list(self.data.models[baseline_model].columns)
            + ["spoortakmodel"]
        )
        ...
        for spoortakmodel_version, model in self.data.models.items():
            if spoortak_identifier in model.index:
                ...
                model_data = pd.concat(
                    [model_data, pd.DataFrame(df).T], axis=0
                )
        pprint(model_data)
        ...
        change_data = pd.DataFrame(
            columns=list(self.data.model_changes[baseline_model].columns)
            + ["spoortakmodel"]
        )

        for (
            spoortakmodel_version,
            model_changes,
        ) in self.data.model_changes.items():
            df = model_changes[
