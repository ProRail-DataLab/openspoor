...
class SpoortakModelMapper:
    ...
    def _is_new_spoortak(self, spoortak_identifier, spoortak_model: int):
        ...
        action_new = changes[
            (changes["FWENAME"] == spoortak_identifier)
            & (changes["ACTION"] == "NEWTOP")
        ]
        return len(action_new) > 0

    def _retrieve_spoortak(
        self, spoortak_identifier: str, model_version: int
    ) -> Optional[pd.Series]:
    ...
    def _related_spoortakken(
        ...
        for model_version in self._data.model_version_numbers:
            ...
            if len(changes) > 0:
                ...
                temp_related_spoortakken.update(
                    changes["DASSIGNNAME"].unique()
                )
            ...
            for entry in temp_related_spoortakken:
                ...
                if (
                    spoortak["GEOCODE_BEGIN"] in geocodes
                    or spoortak["GEOCODE_EIND"] in geocodes
                ):
    ...
    def map(
        ...
        for model_version in self._data.model_version_numbers[::-1]:
            ...
            spoortak_data = self._retrieve_spoortak(
            ...
            found_subsections.append(
                SpoortakSubsection(
                    str(spoortak_data.name),
                    max(
                        spoortak_subsection.kilometrering_start,
                        spoortak_data["kilometrering_start"],
                    ),
                    min(
                        spoortak_subsection.kilometrering_end,
                        spoortak_data["kilometrering_end"],
                    ),
                    model_version,
                )
            )

            if self._is_new_spoortak(
                spoortak_subsection.identification, model_version
            ):
                log.info(
        ...
        for related_spoortak, related_model_version in related_spoortakken:
            ...
            related_segments = self.map(
                spoortak_subsection,
                _ignore_list + [spoortak_subsection.identification],
            )
            found_subsections.extend(related_segments)

        limited = self._limit_start_end(
            found_subsections,
            spoortak_subsection.kilometrering_start,
            spoortak_subsection.kilometrering_end,
        )
        ...
        ordered = sorted(
            cleaned,
            key=lambda x: (x.spoortak_model_version, x.kilometrering_start),
        )
    ...
    def map_to(
        ...
        return [
            ...
            if result.spoortak_model_version == model_version
        ]
