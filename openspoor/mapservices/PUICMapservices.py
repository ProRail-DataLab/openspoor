...
class PUICMapservices:
    ...
    def __init__(
        self, spoor_cache_location=None, wisselkruisingbeen_cache_location=None
    ):
        """
        :param spoor_cache_location: filepath as in MapservicesData class
        """
        logger.info(
            "Initiating PUICMapservices object in order to obtain "
        ...
        self.wisselkruisingbeen_query = featureserver.search_for(
            "wissel kruisingbeen", exact=True
        )

        if spoor_cache_location:
            self.spoor_query.write_gpkg(spoor_cache_location)
        if wisselkruisingbeen_cache_location:
            self.wisselkruisingbeen_query.write_gpkg(
                wisselkruisingbeen_cache_location
            )
    ...
    def _combine_spoor_and_wisselkruisingbeen(
        self, spoor_gdf, wisselkruisingbeen_gdf
    ):
        """
        Combine the spoor_gdf with the wisselkruisingbeen_gdf with correct
        columns
        """
        all_data_gdf = pd.concat(
            [spoor_gdf, wisselkruisingbeen_gdf], sort=True
        )
