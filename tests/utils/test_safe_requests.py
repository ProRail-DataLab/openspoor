...
@pytest.fixture
def count_url():
    return 'http://mapservices.prorail.nl/arcgis/rest/services/Kadastraal_004/MapServer/5/query' \
...
@pytest.fixture
def input_json():
    return {'features': [{'geometry': {'coordinates': [112734.526, 480849.498], 'type': 'Point'},
                          'properties': {'FID': '0'}, 'type': 'Feature'}],
            'name': 'RD_Coordinaten', 'type': 'FeatureCollection'}
...
def test_post_json_success(base_safe_requests, geocode_to_xy_url, input_json):
    assert isinstance(base_safe_requests.get_json('POST', geocode_to_xy_url, input_json), dict)
