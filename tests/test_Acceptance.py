...
from openspoor.mapservices import MapServicesQuery, PUICMapservices
from openspoor.transformers import (
...
mock_spoordata_gdf = gpd.GeoDataFrame(
    {
        ...
        "GUID": [
            "7be2c50d-7139-fb4e-f46a-9900f32e12f7",
            "cb4da2ca-2209-962f-06de-40df9c2f1367",
            "0b3bc0b5-4b93-45a9-9130-12a9907f5940",
            "ee48e124-70e6-89d2-9a4d-38c24339f4bd",
            "330c2647-a053-fd04-0e68-a7634cf52a6a",
            "8713869e-d33f-0db0-ece9-18510ed8530a",
    ...
    geometry=[
        LineString(
            [
                (95672.8720, 433472.6810, 43.0496),
                (95659.391, 433499.2390, 43.07610),
            ]
        ),
        LineString(
            [
                (94460.172, 451056.625, 11.98070),
                (103437.91490, 447295.18030, 2.26510),
            ]
        ),
        LineString(
            [
                (112734.527, 480849.498, 13.56510),
...
class Test:
    ...
    def test_singlequery(self, monkeypatch, tmp_path):
        ...
        spoortak_mapservices = MapServicesQuery(
            url=config["spoor_url"], cache_location=cache_path
        )
    ...
    def test_puicmapservices(self, monkeypatch, tmp_path):
        monkeypatch.setattr(
            "openspoor.mapservices.MapServicesQuery._load_all_features_to_gdf",
            lambda d: self.puic_mock_output,
        )
        cache_path = tmp_path / "puic.p"

        assert ~os.path.exists(cache_path)
        puic_mapservices = PUICMapservices(
            spoor_cache_location=cache_path,
    ...
    def test_acceptance_TransformerCoordinatesToSpoor(
        ...
        pd.testing.assert_frame_equal(output_df, expected_output_df, atol=0.05)

    def test_acceptance_TransformerCoordinatesToSpoor_intersecting_tracks(
        self, coordinates_transformer
    ):
        ...
        pd.testing.assert_frame_equal(
            output_df.sort_values(["NAAM_LANG"]),
            expected_output.sort_values(["NAAM_LANG"]),
            atol=1e-3,
        )
    ...
    def test_acceptance_TransformerSpoortakToCoordinates(self):
        ...
        spoortak_transformer = spoortak_transformer.fit(
            self.spoortak_mock_output
        )
        input_df = pd.DataFrame(
            {
                "SPOOR_ID": ["133_1045BV_13.6", "133_1045BV_13.6"],
                "lokale_kilometrering": [
                    0.002280929575529405,
                    0.10132577461919272,
                ],
            }
        )
