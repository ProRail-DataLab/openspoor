...
MODELS_DATA_DIR = str(
    pathlib.Path(__file__).parents[2].resolve().joinpath("data").resolve()
)
...
class TestSpoortakModelsData(unittest.TestCase):
    ...
    def test_data_dir_exists(self):
        ...
        self.assertTrue(
            os.path.exists(MODELS_DATA_DIR),
            f"{MODELS_DATA_DIR} does not exist",
        )
    ...
    def test_data_unique_spoortak(self):
        ...
        sut = SpoortakModelsData(MODELS_DATA_DIR)
        self.assertFalse(
            any(any(model.index.duplicated()) for model in sut.models.values())
        )
    ...
    def test_rows_loaded(self, model, row_count):
        sut = SpoortakModelsData(MODELS_DATA_DIR)
        self.assertEqual(
            len(sut.models[model]),
            row_count,
            f"model {model} has {len(sut.models[model])} rows",
        )
