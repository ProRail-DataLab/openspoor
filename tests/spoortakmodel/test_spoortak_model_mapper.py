...
from openspoor.spoortakmodel import (
    SpoortakModelMapper,
    SpoortakModelsData,
    SpoortakSubsection,
)
...
class TestSpoortakModelMapper(unittest.TestCase):
    ...
    def test_map_to_split(self):
        ...
        self.assertEqual(
            result[0].kilometrering_end, result[1].kilometrering_start
        )

    @skip(
        "Due to the name swap that happens we currently dont support RENAME."
    )
